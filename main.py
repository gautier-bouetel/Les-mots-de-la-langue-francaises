"""
Code permettant de manipuler les mots de la langue française.
"""
...
def cherche1(mots, start, stop, n):
    """retourne le sous ensemble des mots de n lettres commençant par
    start et finissant par stop
...
def cherche2(mots, lstart, lmid, lstop, nmin, nmax):
    ...
    Returns:
        ...
        dans lstart, contenant une chaine présente dans lmid et finissant par une 
    ...
    for mot in mots:
        ...
        # lstart / lstop peuvent être une liste OU une simple chaîne dans les tests
        starts = lstart if isinstance(lstart, list) else [lstart]
        stops = lstop if isinstance(lstop, list) else [lstop]
        mids = lmid if isinstance(lmid, list) else [lmid]

        if not any(mot.startswith(p) for p in starts):
            continue
        if not any(mot.endswith(s) for s in stops):
            continue

        # la chaîne "mid" doit être présente mais ni au début ni à la fin
        ok_mid = False
        for m in mids:
            pos = mot.find(m)
            if pos != -1 and pos != 0 and pos + len(m) != len(mot):
                ok_mid = True
                break
        if not ok_mid:
            continue
...
def main():
    ...
    print( [ mot for mot in ["chronophage", "procrastinateur",
                             "dangerosité", "gratifiant"] if mot in ens ] )
